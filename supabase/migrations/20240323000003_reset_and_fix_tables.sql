...
-- Create the profiles table first since it depends on auth.users
...
-- Create the cars table with brand reference
create table cars (
  id uuid default gen_random_uuid() primary key,
  make text not null references brands(name) on delete restrict,
  model text not null,
